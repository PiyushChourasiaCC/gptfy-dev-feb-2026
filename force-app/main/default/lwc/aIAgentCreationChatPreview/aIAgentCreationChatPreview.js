...
const INPUT_UPDATE_DELAY_MS = 50;
const FILE_CANCEL_CHECK_DELAY_MS = 300;



export default class AIAgentCreationChatPreview extends NavigationMixin(LightningElement) {
    userId = Id;
    userName;
    userPhotoUrl;
    ...
    errorCallback(error, stack) {
        this.showToast('error', 'Error', 'An unexpected error occurred. Please refresh the page.', 'dismissable');
    }
    @api recordId;
    @api objectApiName;
    @api response;
    ...
    connectedCallback() {
        ...
        // Removed dynamic height calculation - CSS handles height with 75vh
    }
    ...
    disconnectedCallback() {
        this.scrollToBottom();
        // Removed resize listener cleanup - no longer needed
    }

    renderedCallback() {
        this.scrollToBottom();
        // Removed adjustChatHeight() - it was overriding containerHeight with undefined parentHeight
    }
    scrollToBottom() {
    ...
    async endChat(){
        if(this.threadId){
            try{
                ...
                this.expiredChatMessage = this.agenticConfig ? this.agenticConfig.ttlMessage ?  this.agenticConfig.ttlMessage : CHAT_ENDED : CHAT_ENDED;
                this.vectorStoreId = null;
                this.imageFileIds = [];
            }catch(error){
                this.expiredChatMessage = null;
    ...
    async handleNativeFiles(event) {
        try {
            ...
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Upload Failed',
                    message: error?.body?.message || error?.message || 'Failed to upload files',
                    variant: 'error',
                    mode: 'sticky'
                })
            );
